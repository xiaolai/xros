...
argument-hint: "[problem in one line]"
---

# xros:compile — methodology spec compiler

You are compiling one **XROS methodology spec**: a JSON file, conforming to
`${CLAUDE_PLUGIN_ROOT}/schema/xros-spec.schema.json`, that another command
(`xros:run`) will turn into a multi-agent Workflow and execute against a real
verifier. Your job is the *interview*, not the run.

The user's one-line problem (may be empty): **$ARGUMENTS**
...
not their vocabulary. Get a one-line problem statement if `$ARGUMENTS` is empty,
then ask, in order:
...
3. **search** — `approaches` (gate ↓); `independenceRounds` (default 2);
   `maxConcurrentAgents` (note: real Workflow concurrency caps at ~16, higher
   values widen breadth not simultaneity); `crossPollinationRule`;
...
3. **Validate with the bundled validator** — portable, stdlib-only, offline (no
   ...
   inject:
   ```bash
   SCHEMA="${CLAUDE_PLUGIN_ROOT}/schema/xros-spec.schema.json"
   SPEC="xros/specs/<id>.spec.json"   # substitute the real id
   python3 "${CLAUDE_PLUGIN_ROOT}/schema/tools/xros_validate.py" "$SCHEMA" "$SPEC"
   ```
